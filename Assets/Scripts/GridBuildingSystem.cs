...
//GridCell 클래스를 각 그리드 셀의 상태와 데이터를 관리
...
{
    [SerializeField] private int width = 100;     //그리드 가로 크기
    [SerializeField] private int height = 100;    //그리드 세로 크기
    ...
    private void Update()
    {
        ...
        if (lookPosition != Vector3.zero)                //포지션이 0이 아닐 때
        {
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                Vector3Int cellPosition = new Vector3Int(x, 0, z);                         //(0,0,0) (1,0,0) (2,0,0) ~(10,0,10) for 분 돌리면서 배치
    ...
    private Vector3 GetLookPosition()
    {
        ...
        else
        {
            ...
            Ray ray = new Ray(rayOrigin, rayDirection);                                     //만든 위치와 방향으로 ray를 보여준다.
            if (Physics.Raycast(ray, out RaycastHit hitInfo, maxBuildDistance))            //설정한 ray에 물체가 있을 경우
            {
                Debug.DrawRay(ray.origin, ray.direction * hitInfo.distance, Color.blue);    //Scene 창에서 빨간색 선으로 ray를 보여준다.
                return hitInfo.point;                                                      //raycast 된 물체 정보를 리턴한다.
            }
            else
            {
